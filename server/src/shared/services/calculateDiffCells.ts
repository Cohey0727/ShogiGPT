...
/**
 * 2つの局面を比較して、変化があったセルの位置を返す
 */
export function calculateDiffCells(previousBoard: Board, currentBoard: Board): Position[] {
  ...
  for (let row = 0; row < 9; row++) {
    for (let col = 0; col < 9; col++) {
      ...
      // セルの内容が異なる場合
      if (prevCell?.type !== currCell?.type || prevCell?.player !== currCell?.player) {
