 ...
 */
export type PathOf<T, Prefix extends string = ""> = IsAny<T> extends true
  ? Prefix
  ...
  : {
      [Key in keyof T]: NonNullable<T[Key]> extends object
        ?
            | `${Prefix}${Key & string}`
            | `${PathOf<T[Key], `${Prefix}${Key & string}.`>}`
        : `${Prefix}${Key & string}`;
    }[keyof T];
...
export type SnakeToCamelCaseNested<T> = T extends Array<infer U>
...
export type CamelToSnakeCaseNested<T> = T extends Array<infer U>
